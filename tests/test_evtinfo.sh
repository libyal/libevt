...
TEST_EXECUTABLE="../evttools/evtinfo";

if ! test -x "${TEST_EXECUTABLE}";
then
	TEST_EXECUTABLE="../evttools/evtinfo.exe";
fi
		...
		do
			...
			do
				...
				then
					...
					run_test_on_input_file "${TEST_SET_DIRECTORY}" "evtinfo" "with_stdout_reference" "${OPTION_SET}" "${TEST_EXECUTABLE}" "${INPUT_FILE}" "${PROFILE_OPTIONS[@]}" "${OPTIONS[@]}";
					RESULT=$?;
			...
			then
				run_test_on_input_file "${TEST_SET_DIRECTORY}" "evtinfo" "with_stdout_reference" "" "${TEST_EXECUTABLE}" "${INPUT_FILE}" "${PROFILE_OPTIONS[@]}";
				RESULT=$?;
			fi
