 ...
 */
int evt_test_single_open_close_file(
     ...
     int expected_result )
{
	...
	if( access_flags == LIBEVT_OPEN_READ )
	{
		access_string = "read";
	}
	else if( access_flags == LIBEVT_OPEN_WRITE )
	{
		access_string = "write";
	}
	else
 ...
 * Returns 1 if successful, 0 if not or -1 on error
 */
int evt_test_multi_open_close_file(
     libcstring_system_character_t *filename,
     int access_flags,
     int expected_result )
{
...
#endif
{
...
#if defined( HAVE_DEBUG_OUTPUT ) && defined( EVT_TEST_OPEN_CLOSE_VERBOSE )
	 ...
	 */
	if( evt_test_single_open_close_file(
	     source,
	     LIBEVT_OPEN_READ,
	     1 ) != 1 )
	{
	 ...
	 */
	if( evt_test_multi_open_close_file(
	     source,
	     LIBEVT_OPEN_READ,
	     1 ) != 1 )
	{
