...
then
	docker run -t -v "${PWD}:/libevt" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libevt && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libevt;
